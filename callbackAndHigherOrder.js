...
// CODE HERE
function last(arr, callback) { callback(arr[arr.length - 1]); }
...
// CODE HERE 
function contains(arr, name, callback) { callback(arr.includes(name)); }
...
function getUserById(arr, id, callback) {
  let filtered = arr.filter( user => user.id === id)
  callback(filtered[0]);
}
...
// CODE HERE
function addingFactory(num1) { return num2 => num1 + num2; }
